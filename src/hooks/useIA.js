...
export function buildRidePrompt(ride, history, supps, profile) {
  const hist = history.slice(0, 6).map(r =>
    `- ${r.fecha}: ${Math.round(r.dur)}min, ${(r.dist||0).toFixed(1)}km, FC ${Math.round(r.hrAvg||0)}lpm, RPE ${r.rpe}, sensación:${r.sen}, Z4+Z5:${((r.zp||[])[3]||0)+((r.zp||[])[4]||0)}%`
  ).join('\n')
...
export function buildPlanPrompt(rides, supps, profile) {
  const hist = rides.slice(0,8).map(r =>
    `${r.fecha}: ${Math.round(r.dur)}min, RPE ${r.rpe}, sensación:${r.sen}, Z4+Z5:${((r.zp||[])[3]||0)+((r.zp||[])[4]||0)}%`
  ).join('\n')
